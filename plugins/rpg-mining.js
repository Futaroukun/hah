...
`.trim())
    if (user.pickaxe == 0) return m.reply('_Kalo mau mining minimal punya pickaxe_\n\nKetik .craft pickaxe')
    if (new Date - user.lastmining <= cooldown) return m.reply(`
You're already mining!!, please wait *🕐${timers.toTimeString()}*
`.trim())
    const rewards = reward(user)
    let text = 'Kamu berhasil menambah dan kehilangan:'
    ...
    conn.sendButton(m.chat, text, global.wm, './media/mining.jpg', [
[`Inventory`, '.inv'],
[`Menu`, '.menu']
], m, {asLocation: true})
}
