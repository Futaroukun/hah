 ...
 if (teks == 'edukasi') tags = {
  ...
  try {
    ...
    let mpt = clockString(_mpt)
      const sections = [
   {
	title: `${htki} ▮Status 」 ${htka}`,
	rows: [
	    {title: `📛)ഒ Info Bot`, rowId: ".botinfo", description: "✧ Info nya Hu Tao Bot-MD ><"},
	...
	{title: `⚡)ഒ Speed`, rowId: ".speed", description: "✧ Kecepatan Respon Hu Tao Bot-MD..."},
	]
    },{
	title: `${htki} ▮Category 」 ${htka}`,
	rows: [
    ...
    {title: `⫹🎶 › 𐐪-〚 Sound 〛-𐑂`, rowId: ".soundmenu", description: "╰► dengerin lagu nih haha >_<"},
	...
	rows: [
	  ...
	  {title: `🌸 ∫ » Group «`, rowId: ".groupbot", description: '✧ Jangan lupa mampir ya...'},
		{title: `👥 ∫ » Thanks To «`, rowld: ".tqto", description: '✧ Orang Yang Udah Ngebantu Banget Nih... >.<'},
      ]
    },
]
...
let tek = `╭─────═[ *INFO USER* ]═─────⋆
│☂︎ *Name:* *${await conn.getName(m.sender)}*
│☂︎ *Premium:* *${usrs.premiumTime > 1 ? 'Yes': 'No'}* 
│☂︎ *Status:* *${m.sender.split`@`[0] == nomorown ? 'Developer' : (usrs.premiumTime >= 1 ? 'Premium User' : 'Free User')}* 
┠─────═[ *TODAY* ]═───────⋆
│☂︎ *Wib:* *${moment.tz('Asia/Jakarta').format('HH:mm:ss')}*
│☂︎ *Wit:* *${moment.tz('Asia/Jayapura').format('HH:mm:ss')}*
│☂︎ *Wita:* *${moment.tz('Asia/Makassar').format('HH:mm:ss')}*
│☂︎ *Date:* *${new Date().toLocaleDateString()}*
┠─────═[ *DATABASE* ]═─────⋆
│☂︎ *Uptime:* ${mpt}
│☂︎ *Register:* *${Object.keys(global.db.data.users).length}*
│☂︎ *Unregister:* *${Object.values(global.db.data.users).filter(user => user.registered == true).length}*
│☂︎ *Baileys:* Multi Device
│☂︎ *Build Version:* *V.2.1.1*
│☂︎ *Versi:* *Release Version*
│☂︎ *Os Version:* *${os.platform()}*
┠─────═[ *INFO CMD* ]═─────⋆
│ *Ⓟ* = Premium
│ *Ⓛ* = Limit
╰──────────═┅═────────⋆`
const listMessage = {
  text: tek,
  footer: '© HU TAO BOT-MD | ALL RESERVED',
            ...
            templateButtons: [
                {
                    urlButton: {
                        displayText: `${namebot}`,
                        url: 'https://chat.whatsapp.com/FUwgRCtSjYpCZhPOPxlVJS'
                    }
                },
                {
                    urlButton: {
